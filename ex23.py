...
Original Author: Neil Schemenauer <nas@artrix.com>
...
def plot(inputs, outputs, actual):
	...
	points. Useful for visualizing the error of the neural network attempt
...
class NN:
	def __init__(self, ni, nh, no, regression = False)
		...
		self.ni = ni + 1 # +1 for bias nodes
		self.nh = nh + 1 # +1 for bias nodes
		self.no = no
	...
	def train(self, patterns, iterations = 1000, N = 0.5, M = 0.1, verbose = False)
		...
		M is the moment factor.
		"""
		for i in xrange(iterations):
			error = 0.0
			for p in patterns:
				self.update(p[0])
				tmp = self.backPropogate(p[1], N, M)
				error += tmp
